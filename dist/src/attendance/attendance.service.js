...
let AttendanceService = class AttendanceService {
    ...
    async markAttendance(markAttendanceDto, userId) {
        ...
        if (session.class.latitude && session.class.longitude) {
            ...
            const isLocationValid = (0, location_utils_1.verifyLocation)(latitude, longitude, session.class.latitude, session.class.longitude, session.class.locationRadius || 9.144);
            if (!isLocationValid) {
                const R = 6371e3;
    ...
    async clockOut(clockOutDto, userId) {
        ...
        if (session.class.latitude && session.class.longitude) {
            ...
            if (!isLocationValid) {
                ...
                const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
                const distance = R * c;
                throw new common_1.BadRequestException(`Location verification failed. ${(0, location_utils_1.getLocationAccuracyMessage)(distance, session.class.locationRadius || 9.144)}`);
            }
        }
